...
class ConservedWaters(Frame):
    ...
    def makeWindow(self):
        ...
        Label(frame1, text="Probability cutoff").grid(row=4, column=0, sticky=W)
        Button(frame1,text=" Help  ",command=prob_help).grid(row=4, column=2, sticky=W)
        ...
        Button(frame2,text=" Find Conserved Water Molecules ",command= lambda: FindConservedWaters(str(v1.get()).lower(),str(v2.get()).upper(),int(v3.get()),float(v4.get()),float(v5.get()))).grid(row=0, column=1, sticky=W)
...
def resolution_help():
   tkMessageBox.showinfo(title = 'Structure resolution cutoff', message = """All the protein structures to be superimposed will be filetered first according to the structure resolution cutoff. Only structures with better resolution than given cutoff will be used further.
...
def displayInputs(selectedStruturePDB,selectedStrutureChain,seq_id,resolution,prob):
...
def makePDBwithConservedWaters(ProteinsList, temp_dir, outdir):
    ...
    if water_coordinates:
        ...
        if len(water_coordinates)<20000 and len(water_coordinates) != 1:
            ...
            FD = hcluster.fclusterdata(water_coordinates, t=1.5, criterion='distance', metric='euclidean', depth=2, method='single')
            FDlist = list(FD)
            fcDic = {}
...
def filterbyResolution(pdbChainsList,resolutionCutoff = 2.0):
    pdbsList = []
...
def FindConservedWaters(selectedStruturePDB='3qkl',selectedStrutureChain='A',seq_id=95,resolution=2.0,prob=0.7):# e.g: selectedStruturePDB='3qkl',selectedStrutureChain='A'
    online_pdb_db = 'http://www.pdb.org/pdb/files/%s.pdb'
    #displayInputs(selectedStruturePDB,selectedStrutureChain,seq_id,resolution,prob)
    tmp_dir = './temp/'
    ...
    print 'Filtered pdbChainsList is : '
    print pdbChainsList
    for pdbChain in pdbChainsList:
...
#FindConservedWaters('1aay','A',95,2.7)
################################################################################
################################################################################
